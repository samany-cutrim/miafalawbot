...
## Deploy no Render (principal + fallback) e failover via Cloudflare

O bot roda em dois deploys independentes no Render:

- Principal: https://mia-falaw-bot-ngs5.onrender.com
- Fallback: https://miafalawbot-evjo.onrender.com

O diretorio `cloudflare/` contem um Cloudflare Worker que atua como router
de failover: encaminha as requisicoes para o Render principal e, se ele
falhar ou nao responder a tempo, tenta automaticamente o fallback. Depois de
publicar o Worker (`cloudflare/README.md`), aponte `RENDER_URL` em
`apps_script/Codigo.gs` para a URL do Worker em vez da URL do Render
principal.
